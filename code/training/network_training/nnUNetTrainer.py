...
class nnUNetTrainer(NetworkTrainer):
    ...
    def preprocess_patient(self, input_files):
        ...
        preprocessor_class = recursive_find_python_class([join(os.getcwd(), "preprocessing")],
                                                         preprocessor_name, current_module="preprocessing")
        assert preprocessor_class is not None, "Could not find preprocessor %s in preprocessing" %preprocessor_name
        preprocessor = preprocessor_class(self.normalization_schemes, self.use_mask_for_norm,
                                          self.transpose_forward, self.intensity_properties)

        d, s, properties = preprocessor.preprocess_test_case(input_files,self.plans['plans_per_stage'][self.stage][
                                                                 'current_spacing'])
